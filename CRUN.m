...
r1 = 3.9;    % control parameter for logistics map
x0 = 0.5;   % initial value for logistics map
xn = x0;
...
Convergence_curve(1) = Best_Cost; %Replaces first zero of the convergence curve

%% Main Loop of RUN with chaos
...
while it<MaxIt
    ...
    % Update chaos parameter using logistics map
    xn = r1.*xn.*(1-xn);
    ...
    for i=1:nP
            ...
            gama = xn.*(X(i,:)-xn.*(ub-lb)).*exp(-4*it/MaxIt);  
            Stp=xn.*((Best_X-xn.*Xavg)+gama);
            DelX = xn.*(2*xn-1).*abs(Stp);  % Modified using logistic map
